...
export interface CbamHeroEditorProps {
  id?: string;
  content: CbamHeroContent;
  onChange: (updated: CbamHeroContent) => void;
  errors?: Record<string, string>;
  errorPrefix?: string;
}

export const CbamHeroEditor: React.FC<CbamHeroEditorProps> = ({
  id = 'section-hero',
  content,
  onChange,
  errors = {},
  ...
  return (
    <CMSSectionCard
      id={id}
      title="Hero Section"
      subtitle="Header typography, multi-line heading stack and introductory narrative for CBAM Verification"
      icon={FileText}
      badge="Hero Banner"
    ...
    >
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2.5">
            {headingLines.map((line, idx) => {
              const lineError =
                errors[`${errorPrefix}.headingLines.${idx}`] ||
                (idx === 0 && errors[`${errorPrefix}.headingLines`]);
              return (
        ...
        <CMSTextarea
          ...
          optimalMax={240}
          required
          description="High-level narrative introducing EU Carbon Border Adjustment Mechanism (CBAM) compliance and independent verification."
