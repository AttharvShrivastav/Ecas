...
import { CbamHeroEditor } from '../../components/admin/cms/CbamHeroEditor';
import { CbamOverviewEditor } from '../../components/admin/cms/CbamOverviewEditor';
import { CbamRolesEditor } from '../../components/admin/cms/CbamRolesEditor';
import { CbamStatusEditor } from '../../components/admin/cms/CbamStatusEditor';
...
export const AdminCbamVerificationEditorPage: React.FC = () => {
  // Server State
  const [pageId, setPageId] = useState<number | null>(null);
  const [pageTitle, setPageTitle] = useState<string>('CBAM Verification');
  const [updatedBy, setUpdatedBy] = useState<string>('System Default');
  const [updatedAt, setUpdatedAt] = useState<string>('');

  // Form State
  const [formContent, setFormContent] = useState<CbamVerificationPageContent>(defaultCbamVerificationPageContent);
  const [initialContent, setInitialContent] = useState<CbamVerificationPageContent>(defaultCbamVerificationPageContent);
  ...
  const loadPageContent = useCallback(async () => {
    ...
    try {
      const data = await fetchAdminCmsPage<CbamVerificationPageContent>('cbam-verification');
      if (data) {
        setPageId(data.id);
        setPageTitle(data.title || 'CBAM Verification');
        setUpdatedBy(data.updatedBy || 'ADMIN');
        setUpdatedAt(data.updatedAt || new Date().toISOString());

        const merged: CbamVerificationPageContent = {
          seo: { ...defaultCbamVerificationPageContent.seo, ...(data.content?.seo || {}) },
          hero: { ...defaultCbamVerificationPageContent.hero, ...(data.content?.hero || {}) },
          overview: {
            ...defaultCbamVerificationPageContent.overview,
            ...(data.content?.overview || {}),
          },
          roles: {
            ...defaultCbamVerificationPageContent.roles,
            ...(data.content?.roles || {}),
          },
          status: {
            ...defaultCbamVerificationPageContent.status,
            ...(data.content?.status || {}),
          },
          consultingCta: {
            ...defaultCbamVerificationPageContent.consultingCta,
      ...
      console.error('[Admin CMS] Failed to fetch CBAM Verification page:', err);
  ...
  const validateForm = (content: CbamVerificationPageContent): Record<string, string> => {
    ...
    // Overview
    if (!content.overview?.heading?.trim()) {
      errors['overview.heading'] = 'Overview heading is required.';
    }

    // Roles
    if (!content.roles?.heading?.trim()) {
      errors['roles.heading'] = 'Roles heading is required.';
    }

    // Status
    if (!content.status?.heading?.trim()) {
      errors['status.heading'] = 'Status heading is required.';
    }
    if (!content.status?.description?.trim()) {
      errors['status.description'] = 'Status description is required.';
    }
  ...
  const handleSaveChanges = async (e?: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const result = await updateAdminCmsPage<CbamVerificationPageContent>('cbam-verification', formContent);
      if (result) {
        setUpdatedBy(result.updatedBy || 'ADMIN');
        setUpdatedAt(result.updatedAt || new Date().toISOString());
        setInitialContent(formContent);
        setSuccessMessage('CBAM Verification page changes saved and published successfully.');
  ...
  const handleConfirmReset = async () => {
    ...
    setSuccessMessage(null);
    try {
      const result = await resetAdminCmsPage<CbamVerificationPageContent>('cbam-verification');
      if (result) {
        const merged: CbamVerificationPageContent = {
          seo: { ...defaultCbamVerificationPageContent.seo, ...(result.content?.seo || {}) },
          hero: { ...defaultCbamVerificationPageContent.hero, ...(result.content?.hero || {}) },
          overview: {
            ...defaultCbamVerificationPageContent.overview,
  ...
  if (isLoading) {
    return (
      <div className="max-w-6xl mx-auto py-12 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm font-semibold text-[#082046]">Loading CBAM Verification CMS...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20 font-['DM_Sans'] antialiased">
      ...
      <nav className="flex items-center gap-2 text-xs text-[#64748B]">
        ...
        <span>Services</span>
        <span>/</span>
        <span className="font-semibold text-[#082046]">CBAM Verification CMS</span>
      </nav>
      ...
      <div className="bg-white rounded-2xl border border-[#E2E8F0] p-6 shadow-xs space-y-5">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 flex-wrap shrink-0">
            <a
              href="/services/cbam-verification"
              target="_blank"
      ...
      <form onSubmit={handleSaveChanges} className="space-y-6">
        ...
        {/* 2. Overview & Process Workflow Steps */}
        <CbamOverviewEditor
          content={formContent.overview}
          onChange={(overview) => setFormContent({ ...formContent, overview })}
          errors={validationErrors}
        />

        {/* 3. Role of CBAM Verifier */}
        <CbamRolesEditor
          content={formContent.roles}
          onChange={(roles) => setFormContent({ ...formContent, roles })}
          errors={validationErrors}
        />

        {/* 4. Verifier Accreditation Status */}
        <CbamStatusEditor
          content={formContent.status}
          onChange={(status) => setFormContent({ ...formContent, status })}
          errors={validationErrors}
        />
        ...
        <CMSCTAEditor
          ...
          subtitle="Bottom-of-page conversion block guiding clients to emissions verification and CBAM compliance consulting"
        ...
        <CMSSEOEditor
          id="section-seo"
          title="SEO & Metadata"
          subtitle="Search engine indexing, title tags and metadata for CBAM Verification Services"
          defaultTitlePlaceholder="CBAM Verification | ECAS EURO - Emissions Data Assured"
          defaultDescriptionPlaceholder="Independent verification of embedded greenhouse gas emissions data for organisations preparing to meet EU Carbon Border Adjustment Mechanism requirements."
          seo={formContent.seo}
          ...
          <div className="sticky bottom-6 z-30 bg-white/95 backdrop-blur-md p-4 rounded-2xl border border-[#082046]/20 shadow-xl flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-bold text-[#082046]">
                You have unsaved changes in CBAM Verification CMS.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <button
